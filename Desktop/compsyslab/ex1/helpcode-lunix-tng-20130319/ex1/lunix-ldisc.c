...
//	debug("lunix ldisc associated with TTY %s\n", tty->name);
	printk( KERN_INFO "lunix ldisc associated with TTY %s\n", tty->name);
	return 0;
}
...
*/
#endif
//	debug("lunix_ldisc_receive called\n");
#endif
	/*
...
int lunix_ldisc_init(void)
{
	...
	debug("Initializing lunix line discipline\n");
	atomic_set(&lunix_disc_available, 1);
	ret = tty_register_ldisc(N_LUNIX_LDISC, &lunix_ldisc_ops);	/* /drivers/tty/tty_ldisc.c#L83 */
	if (ret)
