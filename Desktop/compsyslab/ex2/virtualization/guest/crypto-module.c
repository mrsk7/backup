                                   ...
                                   struct crypto_vq_buffer *buf)
{
	...
	case VIRTIO_CRYPTO_DEVICE_HOST_OPEN:
		crdev->fd = cpkt->value;
		if (crdev->fd < 0) printk(KERN_WARNING "Failed opening cryptodev at host\n");
		else  printk(KERN_WARNING "fd=%d\n",crdev->fd);
		wake_up_interruptible(&crdev->c_wq);
		break;
	case VIRTIO_CRYPTO_DEVICE_HOST_CLOSE:
		if (cpkt->value < 0) printk(KERN_WARNING "Error@Closing fd in host machine\n");
		else printk(KERN_INFO "OK@Close fd in host machine\n");
		break;
	default:
		return;
	}
