...
static void handle_control_message(VirtIOCrypto *crdev, void *buf, size_t len)
{
	...
	if (cpkt.event == VIRTIO_CRYPTO_DEVICE_GUEST_OPEN) {
		...
		if (cpkt.value) {
			...
			/* ? */
			if(close(crdev->fd)<0) send_control_event(crdev,VIRTIO_CRYPTO_DEVICE_HOST_CLOSE,-1);	//TODO Needs close error checking
			else send_control_event(crdev,VIRTIO_CRYPTO_DEVICE_HOST_CLOSE,0);
		}
	}
	FUNC_OUT;
}
                                          ...
                                          ssize_t len) 
{
	...
	case CIOCCRYPT:
		...
		for (i=0;i<cr_data->op.crypt.len;i++) printf("%x",cr_data->op.crypt.src[i]);
		cr_data->op.crypt.dst=cr_data->dstp;
		printf("\n\n\n");
		for (i=0;i<cr_data->op.crypt.len;i++) printf("%x",cr_data->op.crypt.dst[i]);
	...
	/* ? */
	ret = send_buffer(crdev, cr_data, sizeof(cr_data));
	FUNC_OUT;
	return ret;
}
...
VirtIODevice *virtio_crypto_init(DeviceState *dev)
{
	...
	crdev = DO_UPCAST(VirtIOCrypto, vdev, vdev);  	//Casts from first member of a struct to that struct
							// Here casts from VirtIODevice to VirtIOCrypto
